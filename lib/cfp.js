...
const saveTweetsHashtags = async (formattedTweets, db = null) => formattedTweets.reduce(
  (currentPromise, tweet) => currentPromise.then(() => saveTweetHashtags(
    tweet,
    db,
  )),
  Promise.resolve(),
...
const processHashtags = async (db = null) => {
  ...
  calculated = aliasesResult.docs.reduce(
    (acc, doc) => {
      ...
      if (acc[data.from]) {
        nextAcc = { ...acc };
        if (nextAcc[data.hashtag]) {
          nextAcc[data.hashtag] += nextAcc[data.from];
        } else {
          nextAcc[data.hashtag] = nextAcc[data.from];
        }
  ...
  const results = await Promise.all(Object.keys(calculated).map(async (key) => {
    const newCount = await saveHashtagCount(key, calculated[key], useDB);
    return newCount;
  }));
